    ...
    if hps.data.n_speakers == 0:
        for line in tqdm(open(test_file, "r").readlines()[:50]):
            ...
            with torch.no_grad():
                ...
                y, sr = load_wav_to_torch(wav_fn)
                y = y / 32768.0
                spec = spectrogram_torch(y.unsqueeze(0),
                    1024, sr, 256, 1024).cuda()  # [b, h, t]
                spec_lengths = torch.LongTensor([spec.size(-1)]).cuda()
                mel = net_g(x_tst, x_tst_lengths, spec, spec_lengths)[0]     # [b, h, t]
                audio = vocoder.forward(mel)[0, 0].cpu().numpy()    # [t * hop]
                output_file = os.path.join(output_dir, os.path.basename(item_id) + '.posterior.wav')
                write(output_file, 22050, audio)
