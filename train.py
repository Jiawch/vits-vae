...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(tqdm(train_loader)):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z_q_memory, z_p_memory), (attn_q, attn_p) = net_g(x, x_lengths, spec, spec_lengths)
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        image_dict = { 
            ...
            "all/mel_gen": utils.plot_spectrogram_to_numpy(y_hat[0].data.cpu().numpy()), 
...
def evaluate(hps, generator, eval_loader, writer_eval):
    generator.eval()
    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths) in enumerate(tqdm(eval_loader)):
    ...
    image_dict = {
      "gen/mel": utils.plot_spectrogram_to_numpy(y_hat[0].cpu().numpy())
    }
    if global_step == 0:
